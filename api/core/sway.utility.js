...
function Configuration (options) {
    ...
    this.load = function (configName, path, options) {
        ...
        try {
            ...
            console.log('Error: Configuration.Load(' + configName + '): ' + err.message, 'Configuration');
        }
            ...
            try {
                watchers[configName] = fs.watch(path, {persistent: true}, function (event, filename) {
                    if (event == 'change') {
                        ...
                        console.log(filename + ' reloaded due to ' + event + ' event on file ' + filename, 'Configuration');
                    }
                });
            } catch (err)
            {
                console.log('Error: Exception while watching configuration: ' + filename + ' - ' + err.message, 'Configuration');
            }
        }
    };
