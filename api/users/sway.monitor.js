...
module.exports = function (sway) {
    sway.monitor = {
        ...
        takeSample: function () {
            ...
            if (!sway.monitor.currentFrame) sway.monitor.currentFrame = new Sample();
            ...
            if (sampleFrame != this.currentFrame.id) {
                ...
                this.currentFrame = new Sample(sampleFrame, stamp);
            }
        ...
        middleware: {
            getMonitor: function (req, res) {
                var response = {
                    samples: sway.monitor.samples
                };
                res.status(200).json(response);
            },
            sample: function (req, res, next) {
                // TODO: Include this instrumentation
                if (sway.monitor) sway.monitor.takeSample();
...
// encapsulates the setAverage logic; it's simple, but don't want it hanging around for someone to modify for no good reason
    Sample.prototype.setAverage = function () {
        var count;
...
// encapsulates the setAverage logic; it's simple, but don't want it hanging around for someone to modify for no good reason
    Sample.prototype.setDuration = function () {
        this.durations.push(Date.now() - sway.monitor.previousTime);
    };

// Set request interval data
    Sample.prototype.setIntervals = function (currentTime) {
